  ...
  return (
    <div className={`custom-node process-step-node ${selected ? 'selected' : ''}`}>
      ...
      <div className="node-content">
        <div className="node-title">{data.stepConfig?.displayName || data.displayName || "Unnamed Process"}</div>
    ...
    >
      ...
      <div className="diamond-shape" style={{ top: `${fixedDiamondTop}px`, left: '50%', marginTop: '0px', marginLeft: '-60px' }}>
        <div className="diamond-content">
          <div className="node-title">{data.stepConfig?.displayName || data.displayName || "Unnamed Decision"}</div>
        </div>
      </div>
...
export const AssignNode = ({ data, selected }) => {
  ...
  return (
    <div className={`custom-node assign-step-node ${selected ? 'selected' : ''}`}>
      <Handle type="target" position={Position.Left} />
      <div className="node-content">
        <div className="node-title">{data.stepConfig?.displayName || data.displayName || "Unnamed Assign"}</div>
    ...
    <div className={`custom-node release-step-node ${selected ? 'selected' : ''}`}>
      ...
      <div className="dispose-shape">
        <div className="dispose-content">
          <div className="node-title">{data.stepConfig?.displayName || data.displayName || "Unnamed Release"}</div>
          <div className="node-details">
    ...
    <div className={`custom-node create-step-node ${selected ? 'selected' : ''}`}>
      <div className="create-shape">
        <div className="create-content">
          <div className="node-title">{data.stepConfig?.displayName || data.displayName || "Unnamed Create"}</div>
